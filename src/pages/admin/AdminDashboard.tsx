...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [m, p, l, g] = await Promise.all([
        ...
        client.from('brand_posts').select('id', { count: 'exact', head: true }),
        client.from('brand_lectures').select('id', { count: 'exact', head: true }),
        client.from('brand_gallery').select('id', { count: 'exact', head: true }),
      ]);
  ...
  const loadContent = useCallback(async () => {
    ...
    const table = contentType === 'posts' ? 'brand_posts' : contentType === 'lectures' ? 'brand_lectures' : 'brand_gallery';
          ...
          <>
            <div className="admin-toolbar-v2">
              <select value={contentType} onChange={(e) => { setContentType(e.target.value as 'posts' | 'lectures' | 'gallery'); setContentPage(1); }}>
                <option value="posts">{ko ? '게시글' : 'Posts'} (brand_posts)</option>
                <option value="lectures">{ko ? '강의자료' : 'Lectures'} (brand_lectures)</option>
                <option value="gallery">{ko ? '갤러리' : 'Gallery'} (brand_gallery)</option>
              </select>
